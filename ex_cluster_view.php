...
$tpl = new TemplatePower( template("ex_cluster_view.tpl") );
...
# Summary graphs
#
$graph_args = "c=$clustername&amp;$get_metric_string&amp;st=$cluster[LOCALTIME]";
...
if (!$showhosts) {
	$showhosts = $ex_list[$clustername];
	$newgroup = 1;
	$showhosts = array();
}
foreach ( $showhosts as $host => $value )
   {
      $tpl->newBlock ("sorted_list");
      $host_url = rawurlencode($host);

      $host_link="\"?t=list&c=$clustername&amp;h=$host&amp;$get_metric_string\"";
      $textval = "";
      ...
      if ($value['ALIVE'] == 'DOWN')
      ...
      else
         {
            ...
            elseif ($val['TYPE']=="string" or $val['SLOPE']=="zero" or
               ...
               {
		  if ($newgroup) $cluster_url = $value['CLUSTERNAME'];
